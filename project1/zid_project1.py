...
def get_tics(pth):
    ...
    with open(pth, mode='r') as tem:
        ...
        for i in line:
            ...
            exchange.append(x.lower())
            ticker.append(y.lower())
    dic = dict(zip(ticker, exchange))
    return dic
...
def read_dat(tic):
    ...
    dat_name = tic.lower() + '_prc.dat'  # aal_prc.dat
    dat_location = os.path.join(DATDIR, dat_name)
    with open(dat_location, mode='r') as temp:
        i = temp.readlines()
        li = []
        for lines in i:
            li.append(lines.replace('\n', '')) #lines[0:80]  #li.append(lines.strip())
        return li
...
def line_to_dict(line):
    ...
    templist = [0]
    templist2 = []
    for i in COLWIDTHS.values():
        temp += i
        templist.append(temp)
    # print(templist)
    for j in range(0, len(templist) - 1):
        templist2.append(line[templist[j]:templist[j + 1]])
    newdic = dict(zip(COLUMNS, templist2))
    return newdic
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for i in tickers_lst:
        ...
        data_list = []
        combinationofcollst_dict = {}
        tempdict = {}

        dat_list = read_dat(i)
        for j in dat_list:
            # print(j)
            eachlineofdat_dict = line_to_dict(j)
            # print(tempdict)
            for k in col_lst:
                # print(k)
                combinationofcollst_dict[k] = eachlineofdat_dict[k]
                # print(combinationofcollst_dict)
            data_list.append(combinationofcollst_dict)
            combinationofcollst_dict = {}
        # print(data_list)

        tempdict['exchange'] = tic_exchange_dic[i]
        # print('label:',tic_exchange_dic[i])
        tempdict['data'] = data_list
        # print(tempdict)
